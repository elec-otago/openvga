/***************************************************************************
 *                                                                         *
 *   RAMB16_S9_S18.v - Simulates the Xilinx primitive of the same name for *
...
`define	ST_IDLE		3'b0xx
`define	ST_READ		3'b100
`define	ST_WRITE	3'b110
`define	ST_RESET	3'b1x1
	...
	begin
		case ({ENA, WEA, SSRA})
		`ST_READ:
			DOA <= get_bytea (ram_block_16k[ADDRA[10:2]], ADDRA[1:0]);
	...
	begin
		case ({ENB, WEB, SSRB})
		`ST_READ:
			DOB <= get_wordb (ram_block_16k[ADDRB[9:1]], ADDRB[0]);
			
		`ST_WRITE:
			ram_block_16k[ADDRB[9:1]] <= set_wordb (ram_block_16k[ADDRB[9:1]], ADDRB[0], DIB);
...
/*	//	Port A
	always @(posedge CLKA)	//	Retrieve byte data and parity bit
	begin
		if (ENA)
		begin
			//DOA <= #2 get_byte(ram_block_16k[ADDRA[10:2]], ADDRA[1:0]);
			DOA		<= byte0;
			DOPA	<= get_bit(ram_block_2k[ADDRA[10:2]], ADDRA[1:0]);
// 			if (WEA)
// 			begin
// 				ram_block_2k[ADDRA[10:2]] <= #2 set_bit(ram_block_2k[ADDRA[10:2]], ADDRA[1:0], DIPA);
// 				ram_block_16k[ADDRA[10:2]] <= #2 set_byte(ram_block_16k[ADDRA[10:2]], ADDRA[1:0], DIA);
// 			end
		end
	end
	*/
...
/*	//	Port B
	always @(posedge CLKB)	//	Retrieve byte data and parity bit
	begin
		if (ENB)
		begin
			//DOB <= #2 get_byte(ram_block_16k[ADDRB[10:2]], ADDRB[1:0]);
			DOB		<= byte1;
			DOPB	<= get_bit(ram_block_2k[ADDRB[10:2]], ADDRB[1:0]);
// 			if (WEB)
// 			begin
// 				ram_block_2k[ADDRB[10:2]] <= #2 set_bit(ram_block_2k[ADDRB[10:2]], ADDRB[1:0], DIPB);
// 				ram_block_16k[ADDRB[10:2]] <= #2 set_byte(ram_block_16k[ADDRB[10:2]], ADDRB[1:0], DIB);
// 			end
		end
	end
	*/
