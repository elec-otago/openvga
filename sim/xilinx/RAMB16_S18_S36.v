/***************************************************************************
 *                                                                         *
 *   RAMB16_S18_S36.v - Simulates the Xilinx primitive of the same name    *
...
// Port A.
wire	[31:0]	doa	= ram_block_16k	[ADDRA[9:1]];
wire	[3:0]	dopa	= ram_block_2k	[ADDRA[9:1]];
wire	[31:0]	wr_dat	= ADDRA[0] ? {DIA, doa[15:0]} : {doa[31:16], DIA};
wire	[3:0]	wr_par	= ADDRA[0] ? {DIPA, dopa[1:0]} : {dopa[3:2], DIPA};
always @(posedge CLKA)
	if (SSRA)
		{DOPA, DOA}	<= #2 0;
	else if (ENA) begin
		DOA	<= #2 ADDRA[0] ? doa[31:16] : doa[15:0] ;
		DOPA	<= #2 ADDRA[0] ? dopa[3:2] : dopa[1:0] ;
		
		if (WEA) begin
			ram_block_16k	[ADDRA[9:1]]	<= #2 wr_dat;
			ram_block_2k	[ADDRA[9:1]]	<= #2 wr_par;
		end
	end
